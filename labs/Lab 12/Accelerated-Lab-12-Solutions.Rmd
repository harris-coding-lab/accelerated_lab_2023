---
title: "Lab 12 Solved: Joins and Data Visualization"
...
*Solution: The band is the primary key or band+origin. Many bands can come from the same city*
...
# outside of aes() R doesn't know to look for category or state as column names

ggplot(storms, aes(wind, pressure)) + 
  geom_point(aes(color = "blue", shape = "triangle"))
# here we define color as aes() it treats "blue" like data
```
...
storms |> 
  ...
  ggplot(aes(x = lat, y = long, color = status, group = name)) + 
    geom_point() + 
    geom_path()
```
...
SOLUTION: geom_smooth tries to run the model separately by the color and shapes as subgroups which it cannot handle.
...
```
  a. Is the input to table a vector or a data frame? **a vector!**
  b. Is the output a vector, a data frame or something distinct? **it's a named vector!**
