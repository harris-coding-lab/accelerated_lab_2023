---
title: "TA Session 13: Writing Functions (solutions)"
...
SOLUTION: The only argument of the function is `x`; and the body is `x ^ 2 + 2 * x + 1`**

```{r}
## Vectorization
my_vector <- -10:10

calc_quadratic(my_vector)
```
...
SOLUTION: You get the same results as `1:5`. This is because when you demean all the vectors are identical.
  
  **b. What happens when your input vector is c(1,2,3,4,5, NA)? Rewrite the function so the result is:^[Hint: take advantage of `mean` and  `sd` NA handling.]**
...
**Q2. Wow! The observed mean is almost twice what we expected given `true_mean`! Let's calculate a z-score to put that in perspective. (Focus on the formulas, you'll learn the intuition in stats class.^[Technical note: If you can't help but be curious about the stats, what we are doing is logically equivalent to a 1000 [z-tests](https://en.wikipedia.org/wiki/Z-test).]**
...
That outcome seems surprising, but we could also just have made an unusual draw. In this TA session, we want to see how often we get such extreme results. We will repeat the steps above 1000 times each, but first we'll write functions that will make this process smooth!
...
Now check out `do_monte_carlo` below. It's our road map. Check if you realize how each step works. Your processes will be different from ours, but that doesn't mean ours is better.
...
**Q5. Write a function called `get_zscores` that takes the observed means and sds, the true mean and N as inputs and returns a z-score as an output. Name the arguments `obs_mean`, `true_mean`, `obs_sd`, and `N`.**
...
**Q6. The function you wrote should also work on vectorized functions. Run the following code which takes estimates of the mean and standard deviation from 5 random draws and returns their associated z-scores:**
...
**Q10. Write the function `make_mc_sample` by completing the following code. The inputs are described above. The output is a tibble with `B` rows of means and standard deviations.**
...
```{r}
do_monte_carlo(N = 30, true_mean = 0.5)
```
...
```{r}
do_monte_carlo(N = 30, true_mean = 0.5, alpha = 0.5)
```
...
```{r}
## Accomodating t-distribution
test_significance <- function(zscores, alpha) {
  
  abs(zscores) > qt(1 - alpha/2, df = N-1)
